...
int main()
{
    ...
    int rfd; // Rendezvous-Descriptor Server, dient zur Communication
    ...
    while (ENDLOSSCHLEIFE) {
        ...
        if(pid2 == 0){
            char subpid[10];
            sprintf(subpid, "%d", getpid());

            for (int i = 0; i < MAXSUBS; i++){
                if(strcmp(subs[i].pid, subpid) == 0){
                    if (strcmp(subs[i].value, get(subs[i].key, speicher))!=0){
                        sprintf(output,"\nsub:%s:%s -> %s\n\n",subs[i].key,subs[i].value,get(subs[i].key, speicher));
                        write(cfd, output, sizeof(output));
                        strcpy(subs[i].value, get(subs[i].key, speicher));
                    }
                }
            }
        }
        ...
        {
            char checkPid[10];
            sprintf(checkPid, "%d", getpid());

            for (int i = 0; i < MAXSUBS; i++){
                if(strcmp(subs[i].pid, checkPid) == 0){
                    if (strcmp(subs[i].value, get(subs[i].key, speicher))!=0){
                        sprintf(output,"\nsub:%s:%s -> %s\n\n",subs[i].key,subs[i].value,get(subs[i].key, speicher));
                        write(cfd, output, sizeof(output));
                        strcpy(subs[i].value, get(subs[i].key, speicher));
...
//            }
    ...
    close(rfd);  // Rendezvous Descriptor schließen
}
